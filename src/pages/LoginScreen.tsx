    ...
    <div className="screen-login">
      <div className="login-wrap">
        ...
        <div>
          ...
          <p className="login-sub">Track goals. Stay accountable. Crush it together.</p>
        </div>
        <form className="login-form" onSubmit={handleSubmit}>
          ...
          <div className="input-group">
            <label>
              Room code <span style={{ color: 'var(--muted2)', fontSize: '0.72em' }}>(share with friend)</span>
            </label>
            <input
              type="text"
              value={room}
          ...
          <div className="input-group">
            ...
            <select value={role} onChange={(e) => setRole(e.target.value)} disabled={availableRoles.length === 1}>
              {availableRoles.includes('you') && <option value="you">Player 1 (You)</option>}
              {availableRoles.includes('friend') && <option value="friend">Player 2 (Friend)</option>}
            </select>
            {availableRoles.length === 1 && (
              <div style={{ fontSize: '0.75rem', color: 'var(--accent)', marginTop: '4px' }}>
                ℹ️ {availableRoles[0] === 'you' ? 'Player 1' : 'Player 2'} already taken - assigned to you
              </div>
            )}
          </div>
          <div className="room-info">
            💡 Both you and your friend open this app, enter the <strong>same room code</strong>, and choose different roles.
